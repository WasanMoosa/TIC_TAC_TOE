...
public class TicTacToe {
	 ...
	 * @param mov the input
	 * @param movepos the array of the position
	 * @param numberPlayer the name of player
	 * @return char the caracter of the input
	 */
	private static char checkInput(char mov, char[] movepos, int numberPlayer) {
		Scanner sc = new Scanner(System.in);
		boolean check = false;
		while (!check) {
			for (int d = 0; d < 9; d++) {
			...
			if (!check) {
				System.out.print("Player " + numberPlayer + ", Invalid input, re-enter position:");
